...
import { fetchJurisdictionPackProfile, fetchJurisdictionPackUtility, getJurisdictionCoverage } from './jurisdictions/registry'
import type { JurisdictionUtilityData, JurisdictionZoningData } from './jurisdictions/types'
...
export type ZoningData = JurisdictionZoningData

export type UtilityCapacityData = JurisdictionUtilityData
...
export function hasLocalCoverage(category: LocalCategory, stateCode: string): boolean {
  if (registry.some((a) => a.category === category && a.stateCode === stateCode)) return true
  const packs = getJurisdictionCoverage().filter((pack) => pack.stateCode === stateCode)
  return category === 'zoningAtlas' ? packs.length > 0 : category === 'utilityCapacity' ? packs.some((pack) => pack.hasUtility) : false
}
...
export function fetchZoningAtlas(coordinates: Coordinates, stateCode: string, signal?: AbortSignal): Promise<ZoningObservation> {
  return fetchJurisdictionPackProfile(coordinates, stateCode, signal).then((result) => (
    result ?? fetchLocalObservation<ZoningData>('zoningAtlas', coordinates, stateCode, ZONING_FALLBACK_PROVENANCE, signal)
  ))
}
...
export function fetchUtilityCapacity(coordinates: Coordinates, stateCode: string, signal?: AbortSignal): Promise<UtilityCapacityObservation> {
  return fetchJurisdictionPackUtility(coordinates, stateCode, signal).then((result) => (
    result ?? fetchLocalObservation<UtilityCapacityData>('utilityCapacity', coordinates, stateCode, UTILITY_FALLBACK_PROVENANCE, signal)
  ))
}
...
export function getLocalCoverageSummary(): Record<LocalCategory, string[]> {
  ...
  for (const pack of getJurisdictionCoverage()) {
    if (!summary.zoningAtlas.includes(pack.label)) summary.zoningAtlas.push(pack.label)
    if (pack.hasUtility && !summary.utilityCapacity.includes(pack.label)) summary.utilityCapacity.push(pack.label)
  }
  return summary
}
