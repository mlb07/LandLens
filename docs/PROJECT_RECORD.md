...
7. ~~Add automated provider-contract tests and browser tests for the critical map-save-compare-report flow.~~ **Done and expanded: 104 unit/provider/component tests plus Playwright parcel lookup → facts → save → compare → report coverage. `npm run verify:providers` checks FEMA, USGS, Austin zoning/energy, and all 12 enriched parcel schemas for renamed or removed fields.**
